...
public class Storage {
	...
	//Builds a string based on contents of the relevant object (Using query builder)
	...
	//Test queries in SQLite or the Manager Swing GUI that comes with HSQLDB if you are unsure.
					...
					try(ResultSet tSet = stmt.executeQuery(tQuery)){
						while(tSet.next()) {
							...
							try (ResultSet iSet = stmt.executeQuery(iQuery)){
								while(iSet.next()) {
									t.getItemsBought().add(new LineItem(iSet.getString("product_id"), iSet.getString("item"),
